...
To use a Hypermode-hosted model, set `host: "hypermode"`, `provider: "hugging-face"`, and set `sourceModel` to be the
model name as specified on Hugging Face.
...
By default, if the model you use is available as a shared model, your app uses these shared models at runtime. You can
override this default behavior by setting `dedicated: true` on your model in the [app manifest](/modus/app-manifest).

If the model you use isn't available as a shared model, Hypermode automatically spins up a dedicated instance of the
...
<note>
  We're constantly evaluating model usage in determining new models to add to our shared catalog. Interested in
  consuming an open source model not listed here by the token? Let us know at
