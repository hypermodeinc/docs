---
title: "Day 18: Retrieval with PostgreSQL - Building RAG Systems"
sidebarTitle: "Day 18: PostgreSQL RAG"
description:
  "Build sophisticated RAG (Retrieval Augmented Generation) systems using
  PostgreSQL and Supabase, implementing semantic search over structured product
  catalogs for intelligent information retrieval."
---
...
- Set up a Supabase PostgreSQL database with vector search capabilities
...
<Warning>
  ...
  PostgreSQL-based RAG system.
</Warning>

## Step 1: Understanding RAG systems

Before building, understand what makes RAG powerful:
...
search **Vector Database**: Stores and searches embeddings efficiently
...
<Tip>
  **RAG thinking**: Think of RAG as giving your agent access to a dynamic,
  searchable library rather than memorizing everything up front.
</Tip>

## Step 2: Set up Supabase with vector search
...
3. **Enable the pgvector extension** for vector similarity search
4. **Set up authentication** and database permissions
...
You'll need to generate embeddings for each product. Here's how to structure the
embedding generation:

```python
# Example Python script to generate embeddings
import openai
import psycopg2
import json
...
print("All products updated with embeddings")
```

<Info>
  **Alternative approach**: Use Supabase's built-in embedding generation
  features or integrate with Hypermode's embedding capabilities for automated
  processing.
</Info>

## Step 4: Connect your agent to Supabase

Now integrate your RAG system with your Hypermode agent:
...
The agent should:
- Use semantic search to find relevant products from our catalog
...
1. When users ask about products, use semantic search on the products database
...
## Step 5: Implement semantic search queries
...
2. **Query the products table** using vector similarity
3. **Return relevant products** with explanations
4. **Format results** with specific details and reasoning
...
<Tip>
  **Retrieval quality**: Good RAG systems return not just similar products, but
  contextually relevant ones that actually answer the user's underlying need.
</Tip>

## Step 6: Advanced retrieval patterns

Enhance your RAG system with sophisticated search capabilities:

### Hybrid search (combining vector and keyword search)

```sql
-- Function for hybrid search combining similarity and text search
...
BEGIN
  ...
  SELECT
    ...
    ts_rank(to_tsvector('english', p.title || ' ' || p.description), plainto_tsquery('english', query_text)) as text_score,
    (1 - (p.embedding <=> query_embedding)) * 0.7 + ts_rank(to_tsvector('english', p.title || ' ' || p.description), plainto_tsquery('english', query_text)) * 0.3 as combined_score
  FROM products p
  WHERE
    (1 - (p.embedding <=> query_embedding)) > match_threshold
    OR to_tsvector('english', p.title || ' ' || p.description) @@ plainto_tsquery('english', query_text)
...
Add filters for better targeted search:

```sql
-- Function with category and price filtering
CREATE OR REPLACE FUNCTION filtered_search_products(
  query_embedding vector(1536),
...
Ensure your RAG system performs well at scale:
...
RAG transforms static agents into dynamic information systems:

**Before RAG**: Agents limited to training data and general knowledge

**After RAG**: Agents with access to current, specific, searchable knowledge
bases
...
```text
Test my RAG system with these different query types:
...
<Tip>
  **Remember**: RAG quality depends on both the relevance of retrieved
