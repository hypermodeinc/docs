...
sidebarTitle: "Day 19: MongoDB RAG"
description:
...
<Card title="Day 19 challenge" icon="file-text">
  **Goal**: build document-based RAG with MongoDB for unstructured data retrieval
...
Document-based RAG excels when dealing with varied, unstructured content that
doesn't fit neatly into database tables.
...
- Implement semantic search across varying document structures
...
### When to use document-based RAG
...
MongoDB Atlas uses Atlas Search for vector capabilities:
...
## Step 3: Load Amazon product reviews data
...
Similar to Day 18, you'll need to generate embeddings:

```python
# Python script to generate embeddings for MongoDB reviews
import pymongo
import openai
import json
...
reviews = collection.find({"embedding": {"$exists": False}})

for review in reviews:
    # Combine review text with product context for better embeddings
    text = f"{review['product_title']}. {review['review_text']}"
...
print("Embeddings generated for all reviews")
```

## Step 4: Connect your agent to MongoDB
...
## Step 5: Implement document search and analysis

Test your document-based RAG system:
...
1. **Search reviews** using semantic similarity for camera-related content
...
## Step 6: Advanced document retrieval patterns

Enhance your MongoDB RAG with sophisticated search capabilities:

### Multi-field semantic search

```javascript
// Advanced aggregation pipeline for semantic search with filters
;[
  {
    $vectorSearch: {
  ...
  {
    $match: {
      rating: { $gte: 4 }, // Filter for positive reviews
      verified_purchase: true,
    },
  },
  {
...
;[
  ...
  {
    $match: {
      sentiment: { $in: ["positive", "mostly_positive"] },
    },
  },
  {
    $group: {
...
```javascript
// Aggregate reviews by mentioned features
;[
  { $match: { features_mentioned: { $exists: true } } },
  { $unwind: "$features_mentioned" },
  {
    $group: {
      _id: "$features_mentioned",
      avg_rating: { $avg: "$rating" },
      mention_count: { $sum: 1 },
      products: { $addToSet: "$product_title" },
      sample_reviews: {
        $push: {
          text: "$review_text",
          rating: "$rating",
...
**Semantic document search**: implemented vector similarity search across
