...
description:
  With a working UI for querying sample data you added, you now need a UI to add
  new posts using GraphQL Mutations in Apollo React.
"og:title": "GraphQL Mutations - Dgraph"
---
...
into the home page's post list. GraphQL has a nice mechanism called _fragments_
...
up a modal UI component (sometimes called a _modal dialog_) to enter the post
data. The user fills in the details of the post, and then the mutation is
submitted when they press _Submit_. This results in a new post, but how does
...
1. Clicking _Submit_ on the new post sends data to the server, and the UI waits
...
![create post button](/images/dgraph/guides/message-board-app/create-post-button.png)

When clicked, this button brings up the modal to create the new post:

![new post modal](/images/dgraph/guides/message-board-app/new-post-modal.png)
