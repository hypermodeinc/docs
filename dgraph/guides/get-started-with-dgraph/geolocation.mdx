...
In the [previous tutorial](./tutorial-7/index.md), we learned about building a
twitter-like user-search feature using
[Dgraph's fuzzy search](./query-language/functions.md#fuzzy-matching).
...
shopping centers within a mile of your current location. Applications like these
make use of your geolocation data.

Geolocation has become an integral part of mobile applications, especially with
the advent of smartphones in the last decade, the list of applications which
revolves around users location to power application features has grown beyond
imagination.
...
than ever before, being able to store and query the location data efficiently
can give you an edge over your competitors.

Real-world data is interconnected; they are not sparse; this is more relevant
when it comes to location data. The natural representation of railway networks,
maps, routes are graphs.

The good news is that [Dgraph](https://dgraph.io), the world's most advanced
graph database, comes with functionalities to efficiently store and perform
useful queries on graphs containing location data. If you want to run queries
like `find me the hotels near Golden Gate Bridge`, or find me all the tourist
location around `Golden Gate Park`, Dgraph has your back.
...
the Eifel tower in Paris. Point locations are useful for representing a precise
...
It is not possible to represent geographical entities which are spread across
...
boundaries, and they are collectively represented using an array of location
...
influence the modeling decisions. The goal of the tutorial is not to arrive at
...
_Note: Dgraph allows you to associate one or more types for the nodes using its
type system feature, for now, we are using nodes without types, we'll learn
about type system for nodes in a future tutorial. Check
...
[first tutorial](/tutorial-1/index.md) to learn the basics of mutations in
Dgraph._
...
_Note: Check the [second tutorial](./tutorial-2/index.md) if you want to learn
more about traversal queries like the above one._
...
- One blue `city node`. We just have one node which represents the city of
  `San Francisco`.
- The green ones are the the `location` nodes. We have a total of 13 locations.
...
_Note: Check out the [previous tutorial](./tutorial-3/index.md) to know more
about data types in Dgraph._
...
the hotels. You can visit our [third tutorial](./tutorial-3/index.md) of the
...
_Note: Refer to the [third tutorial](./tutorial-3/index.md) of the series to
learn more about hash index and comparator functions in Dgraph._
...
We have two hotels in the result, and one of them is over their budget of 300$
per night. Let's add another filter to search for Hotels priced below $300 per
night.
...
- A wider range of queries can also be found in the
  [Query Language](/query-language/_index) reference.
- See [Deploy](./deploy/_index) if you wish to run Dgraph in a cluster.
