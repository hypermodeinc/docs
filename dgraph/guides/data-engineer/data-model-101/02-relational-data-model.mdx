...
description:
  Changing the schema in a relational model directly affects the data that is
  held by the model, and can impact database query performance.
---
...
table. This so-called _pivot table_ usually does not store any information
...
adopts all of the columns from the table. When you add a new property to a
table, the model changes and adds the new property as a column on every existing
and future row for the table. And when you add a new data type to the database,
you create a new table with its own pre-defined columns. This new data type
might link to existing tables or need more pivot tables for a new many-to-many
relationship. So, with each data type added to your relational data model, the
need to add foreign keys and pivot tables increases, making support for querying
every potential data relationship increasingly unwieldy.
...
effects the data that is held by the model, and can impact database query
performance.
