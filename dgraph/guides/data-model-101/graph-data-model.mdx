...
description:
  When modeling a graph, focus on the relationships between nodes. In a graph,
  you can change the model without affecting the underlying data
---
...
string. However, any one node could have other predicates that are not contained
...
`father` and the `mother` are linked together with a `spouse` edge, and both
parents are related to the child along a `child` edge.
...
data. You can insert this whole data object with 3 people, a post, and a
comment; all in one step.
...
define. Other items not specifically given the new property type are not
changed. When you add a new data type to the database, a new node is created,
ready to receive new edges and predicates.
