...
"og:title": "Single Server Cluster Setup - Dgraph"
---
...
1. Verify that you are able to access the nodes in the Kubernetes cluster:
   ...
   [dgraph-single.yaml](https://github.com/hypermodeinc/dgraph/blob/main/contrib/config/kubernetes/dgraph-single/dgraph-single.yaml)
...
2. Start a StatefulSet that creates a single Pod with `Zero`, `Alpha`, and
   ...
   ```sh
   kubectl create --filename https://raw.githubusercontent.com/hypermodeinc/dgraph/main/contrib/config/kubernetes/dgraph-single/dgraph-single.yaml
   ```
...
```sh
kubectl delete --filename https://raw.githubusercontent.com/hypermodeinc/dgraph/main/contrib/config/kubernetes/dgraph-single/dgraph-single.yaml
