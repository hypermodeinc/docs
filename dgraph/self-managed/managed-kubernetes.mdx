...
### Deploy Dgraph Zero (Cluster Management)

<Info>
Dgraph Zero manages the cluster metadata and coordinates the Alpha nodes. It's essential for cluster operations.
</Info>

Create `dgraph-zero.yaml` file with the following content:

```yaml dgraph-zero.yaml
apiVersion: v1
kind: Service
metadata:
  name: dgraph-zero
  ...
  template:
    ...
    spec:
      ...
      - name: zero
        ...
        - |
          set -ex
          dgraph zero --my=$(hostname -f):5080 --replicas=1
...
Deploy the configuration with `kubectl apply -f dgraph-zero.yaml`:

```bash Deploy Zero
kubectl apply -f dgraph-zero.yaml
```
...
```bash
# Wait for Zero to be ready
kubectl wait --for=condition=ready pod -l app=dgraph-zero -n dgraph --timeout=300s
```


### Deploy Dgraph Alpha (Data Nodes)

<Info>
Dgraph Alpha nodes store and serve your data. This configuration creates 3 Alpha replicas for high availability.
</Info>
      ...
      template:
        ...
        spec:
          ...
          - name: alpha
            ...
            - |
              set -ex
              dgraph alpha --my=$(hostname -f):7080 --zero=dgraph-zero.${POD_NAMESPACE}.svc.cluster.local:5080 --whitelist=0.0.0.0/0
            securityContext:
              runAsUser: 0
      volumeClaimTemplates:
      - metadata:
