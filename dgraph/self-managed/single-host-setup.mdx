...
3. Optionally launch [Ratel](./ratel/overview) using the `dgraph/ratel` docker
   image:
...
8. Connect the Dgraph cluster that are running using https://play.dgraph.io/.
...
1. Run two Dgraph Alpha nodea:
   ...
   each Alpha if they're running on the same host. You can use `-p` and `-w` to
