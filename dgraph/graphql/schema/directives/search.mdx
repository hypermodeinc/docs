...
description:
  What search can you build into your GraphQL API? Dgraph builds search into the
  fields of each type, so searching is available at deep levels in a query
---
...
The `@search` directive is used in conjunction with `@embeding` directive to
