...
description:
  Dgraph creates a GraphQL API from nothing more than GraphQL types. To
  customize the behavior of your schema, you can implement custom resolvers.
---
...
- a `body` definition that can be used to construct a HTTP body from from
  arguments or fields.
...
In the above request, `Github-Api-Token` would be sent as a header with value
`long-token` for the introspection request. For the actual `/graphql` request,
the `Authorization` header would be sent with the value `long-token`.
...
- Fields or arguments used in the path of a URL, such as `username` or
  `authorID` in the examples above, must be marked as non-nullable (have `!` in
  their type); whereas, those used in parameters, such as `numToFetch`, can be
  nullable.
...
- Currently, the body can only contain alphanumeric characters in the key and
  other characters like `_` are not yet supported.
- Currently, constant values are not also not allowed in the body template. This
...
`body` pattern that is used to build HTTP request bodies.
...
- If a nullable argument is not present, nothing is added to the URL/body. That
  ...
  would be `https://a.b.c/endpoint` if `gqlArg` were not present in the request
  arguments.
...
When a schema is uploaded, Dgraph will try to introspect the remote GraphQL
endpoints on any custom logic that uses the `graphql` argument. From the results
of introspection, it tries to match up arguments, input and object types to
...
Any type annotated with the `@remote` directive is not stored in Dgraph. This
...
For example, you api might use custom logic to integrate with GitHub, using
...
GraphQL schema will need to include the type you expect back from that remote
call. That could be linking a `User` as stored in your Dgraph instance with the
...
directive on the fields of a `@remote` type in order to map the JSON key
response of a custom query to a GraphQL field.
...
GraphQL error propagation will be triggered.
...
Let's say we have an application about authors and posts. Logged in authors can
add posts, but we want to do some input validation and add extra value when a
post is added. The key types might be as follows.
...
- [Remote Types - Turning off Generated Mutations with `@remote` Directive](./directive/#remote-types)
