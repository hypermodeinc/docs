...
<Note>
  The `get` API on interfaces containing fields with the `@id` directive is
  being deprecated and will be removed in v21.11. Users are advised to use the
  `query` API instead.
</Note>
...
fetches the `title`, `text` and and `datePublished` for all posts:
...
to the field that will be used to filter the results.
...
<Tip>
  ...
  `between`](/query-language/functions/#between).
</Tip>
