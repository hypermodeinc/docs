...
title: Audit Logging
description:
  With an Enterprise license, Dgraph can generate audit logs that let you track
  and audit all requests (queries and mutations).
---

As a database administrator, you count on being able to audit access to your
database. With a Dgraph [enterprise license](./enterprise-features/license), you
can enable audit logging so that all requests are tracked and available for use
in security audits. When audit logging is enabled, the following information is
...
- gRPC requests to Raft endpoints (see [RAFT](./design-concepts/raft))
...
To learn more about the logging features of Dgraph, see [Logging](./log-format).
