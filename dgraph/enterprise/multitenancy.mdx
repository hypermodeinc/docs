...
<Note>
  ...
  administration operations, such as exporting data of all tenants. _Guardian_
  of the _Galaxy_ group cannot read across tenants.
</Note>
...
For example, if the user `rocket` is part of the _Guardians of the Galaxy_ group
...
<Note>
  To create a namespace, the _Guardian_ must send the JWT access token in the
...
By sending the mutation above, a namespace is created. A _Guardian group_ is
...
[`addUser`](./access-control-lists.md#create-a-regular-user).
...
_Guardian of the Galaxy_ you can query `state` from GraphQL:
...
<Note>
  To delete a namespace, the _Guardian_ must send the JWT access token in the
  `X-Dgraph-AccessToken` header.
</Note>
...
Only members of the _Guardians of the Galaxy_ can reset passwords across
...
operations, see [Alter the database](./raw-http.md#alter-the-database).
...
[Initial import](./bulk-loader) and [Live import](/live-loader) tools support
multi-tenancy.
...
If a _Guardian of the Galaxy_ exports the whole Cluster, a single folder
...
For example, when the _Guardian of the Galaxy_ user is used to export the
...
When using the _Guardian of the Namespace_, there's no need to specify the
