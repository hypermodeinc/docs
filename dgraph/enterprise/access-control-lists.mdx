...
<Note>
  ...
  [v1.1.0](https://github.com/dgraph-io/dgraph/releases/tag/v1.1.0). The `dgraph
  acl` command is deprecated and will be removed in a future release. ACL
  changes can be made by using the `/admin` GraphQL endpoint on any Alpha node.
</Note>
...
[dgo](https://github.com/dgraph-io/dgo) or
[dgraph4j](https://github.com/dgraph-io/dgraph4j), must authenticate with a
...
1.  Create a plain text file named `hmac_secret_file`, and store a randomly
...
1.  Start all the Dgraph Alpha nodes in your cluster with the option
    `--acl secret-file="/path/to/secret"`, and make sure that they are all using
    the same secret key file created in Step 1. Alternatively, you can
...
environment variables, i.e. `DGRAPH_ALPHA_ACL="secret-file=</path/to/secret>"`
and `DGRAPH_ALPHA_SECURITY="whitelist=<permitted-ip-addresses>"`. See
[Config](./deploy/config) for more information in general about configuring
Dgraph.
</Tip>
...
- **Go** ([dgo client](https://github.com/dgraph-io/dgo)): example
  `acl_over_tls_test.go`
  ([here](https://github.com/dgraph-io/dgraph/blob/main/tlstest/acl/acl_over_tls_test.go))
- **Java** ([dgraph4j](https://github.com/dgraph-io/dgraph4j)): example
  `AclTest.java`
  ([here](https://github.com/dgraph-io/dgraph4j/blob/master/src/test/java/io/dgraph/AclTest.java))
...
In case you have [reverse edges](./dql-schema.md#reverse-edges), they have to be
given the permission to the group as well
