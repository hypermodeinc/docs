...
description:
  "Complete guide for migrating Dgraph clusters from Dgraph Cloud and Hypermode
  Graph to self-hosted environments across AWS, GCP, Azure, and Digital Ocean"
sidebarTitle: "Self Hosting Guide"
---

## Overview

This comprehensive guide walks you through migrating your Dgraph database from
managed cloud services (Dgraph Cloud or Hypermode Graph) to self-hosted
environments. Whether you're looking to reduce costs, gain more control, or meet
specific compliance requirements, this guide provides detailed instructions for
deploying across multiple cloud providers and deployment methods.
...
### Exporting from Hypermode Graph

Hypermode Graph provides native export capabilities through both API and
interface methods.
...
<Tabs>
  <Tab title="Minimal Downtime Strategy">
    **Recommended for**: Production systems with high availability requirements
    - **Read-only period**: 15-30 minutes - **Full downtime**: 30-60 minutes -
    **Strategy**: Blue-green deployment with data sync - **Tools**: Load
    balancer for traffic switching
  </Tab>

<Tab title="Maintenance Window Strategy">
  **Recommended for**: Systems with scheduled maintenance windows - **Read-only
  period**: 1-2 hours - **Full downtime**: 2-4 hours - **Strategy**: Traditional
  migration during off-peak hours - **Tools**: Application-level maintenance
  mode
</Tab>

  <Tab title="Extended Migration Strategy">
    **Recommended for**: Large datasets or complex environments - **Read-only
    period**: 4-8 hours - **Full downtime**: 8-12 hours - **Strategy**: Bulk
    load with extended testing period - **Tools**: Comprehensive monitoring and
    rollback plan
  </Tab>
</Tabs>
...
<Tabs>
  <Tab title="Kubernetes">
    ```bash kubectl port-forward -n dgraph svc/dgraph-dgraph-alpha 8080:8080 &
    curl -X POST localhost:8080/admin/schema \ -H "Content-Type:
    application/json" \ -d @schema_backup.json ```
  </Tab>

<Tab title="Docker Compose">
  ```bash curl -X POST localhost:8080/admin/schema \ -H "Content-Type:
  application/json" \ -d @schema_backup.json ```
</Tab>

  <Tab title="Linux VPS">
    ```bash curl -X POST 10.0.1.10:8080/admin/schema \ -H "Content-Type:
    application/json" \ -d @schema_backup.json ```
  </Tab>
</Tabs>
...
<Tabs>
  <Tab title="Live Loader (Kubernetes)">
    ```bash kubectl run dgraph-live-loader \ --image=dgraph/dgraph:v23.1.0 \
    --restart=Never \ --namespace=dgraph \ --command -- dgraph live \ --files
    /data/export.rdf.gz \ --alpha dgraph-dgraph-alpha:9080 \ --zero
    dgraph-dgraph-zero:5080 ```
  </Tab>

<Tab title="Live Loader (Docker Compose)">
  ```bash # Copy data files to container docker cp exported_data.rdf.gz
  ...
  <Tab title="Live Loader (Linux VPS)">
    ```bash # Copy data to server scp exported_data.rdf.gz user@10.0.1.10:/tmp/
...
<CardGroup cols={2}>
  <Card
    ...
    icon="book-open"
    href="https://dgraph.io/docs/"
  >
    Complete official documentation for Dgraph database
  </Card>
  <Card
    title="Dgraph GraphQL"
    icon="code"
    href="https://dgraph.io/docs/graphql/"
  >
    GraphQL schema and API documentation
  </Card>
  <Card title="Dgraph Community" icon="users" href="https://discuss.dgraph.io/">
    Official community forum for questions and discussions
  </Card>
  <Card
    title="Dgraph GitHub"
    icon="github"
...
<Tabs>
  <Tab title="AWS">
    - [Amazon EKS Documentation](https://docs.aws.amazon.com/eks/) - [AWS CLI
    ...
    - [AWS VPC CNI
    Plugin](https://docs.aws.amazon.com/eks/latest/userguide/managing-vpc-cni.html)
  </Tab>

<Tab title="Google Cloud">
  - [Google Kubernetes Engine
  (GKE)](https://cloud.google.com/kubernetes-engine/docs) - [gcloud CLI
  Installation](https://cloud.google.com/sdk/docs/install) - [GKE Persistent
  Volumes](https://cloud.google.com/kubernetes-engine/docs/concepts/persistent-volumes)
  - [GKE
...
<Tab title="Azure">
  ...
  (AKS)](https://docs.microsoft.com/en-us/azure/aks/) - [Azure CLI
  Installation](https://docs.microsoft.com/en-us/cli/azure/install-azure-cli) -
  [Azure Disk
  Storage](https://docs.microsoft.com/en-us/azure/aks/azure-disks-dynamic-pv) -
  [Application Gateway Ingress
  Controller](https://docs.microsoft.com/en-us/azure/application-gateway/ingress-controller-overview)
  - [Azure Monitor for
  Containers](https://docs.microsoft.com/en-us/azure/azure-monitor/containers/container-insights-overview)
</Tab>

  <Tab title="Digital Ocean">
    - [DigitalOcean Kubernetes
    (DOKS)](https://docs.digitalocean.com/products/kubernetes/) - [doctl CLI
    Installation](https://docs.digitalocean.com/reference/doctl/how-to/install/)
    - [Block Storage Volumes](https://docs.digitalocean.com/products/volumes/) -
    [Load
    Balancers](https://docs.digitalocean.com/products/networking/load-balancers/)
    - [DigitalOcean
...
<CardGroup cols={2}>
  ...
  >
    Multi-container Docker application definitions
  </Card>
</CardGroup>
...
<AccordionGroup>
  <Accordion title="Prometheus Ecosystem">
    - [Prometheus Documentation](https://prometheus.io/docs/) - [Grafana
    Documentation](https://grafana.com/docs/) - [AlertManager
...
<Accordion title="Logging Solutions">
  - [ELK Stack Documentation](https://www.elastic.co/guide/index.html) -
  [Fluentd Documentation](https://docs.fluentd.org/) - [Loki
  Documentation](https://grafana.com/docs/loki/latest/) - [Vector
  Documentation](https://vector.dev/docs/)
</Accordion>

  <Accordion title="APM and Tracing">
    - [Jaeger Documentation](https://www.jaegertracing.io/docs/) -
    [OpenTelemetry Documentation](https://opentelemetry.io/docs/) - [Zipkin
    Documentation](https://zipkin.io/) - [New Relic
...
#### Hypermode Operations Runbooks

<CardGroup cols={1}>
  <Card
    title="Hypermode Ops Runbooks Repository"
    icon="book"
    href="https://github.com/hypermodeinc/ops-runbooks"
  >
    Repository for operational runbooks - A collection of step by step guides
    for fixing common problems **Contains runbooks for**:
...
#### Community Runbook Resources

<CardGroup cols={2}>
  <Card
    title="Awesome Runbook Collection"
    icon="star"
    href="https://github.com/runbear-io/awesome-runbook"
  >
    A curated list of awesome runbook documents, guidebooks, software and
    resources
  </Card>

<Card
  title="SRE Runbook Template"
  icon="file-alt"
  href="https://github.com/SkeltonThatcher/run-book-template"
>
...
<Card
  ...
  icon="dharmachakra"
  href="https://github.com/openshift/runbooks"
>
  Runbooks for Alerts on OCP - Runbooks in this repository are grouped by the
  operator that is responsible for shipping the respective alert
</Card>

  <Card
    title="CloudOps Automation"
    icon="cloud"
    href="https://github.com/unskript/Awesome-CloudOps-Automation"
  >
    Cloud-ops automation runbooks that are ready to use. Build your own
    automations using the hundreds of drag and drop actions included in the
    repository
  </Card>
</CardGroup>
...
<Tabs>
  <Tab title="Security Best Practices">
    - [CIS Kubernetes
    Benchmark](https://www.cisecurity.org/benchmark/kubernetes) - [NIST
    Cybersecurity Framework](https://www.nist.gov/cyberframework) - [OWASP
    Kubernetes Security Cheat
...
<Tab title="Compliance Frameworks">
  - [SOC 2 Compliance
  Guide](https://www.aicpa.org/interestareas/frc/assuranceadvisoryservices/aicpasoc2report.html)
  - [GDPR Compliance](https://gdpr.eu/) - [HIPAA Security
  Rule](https://www.hhs.gov/hipaa/for-professionals/security/index.html) - [PCI
  DSS Requirements](https://www.pcisecuritystandards.org/pci_security/)
</Tab>

  <Tab title="Security Tools">
    - [Trivy Container Scanning](https://aquasecurity.github.io/trivy/) - [Open
    Policy Agent (OPA)](https://www.openpolicyagent.org/docs/latest/) -
    [Gatekeeper Policy
    Controller](https://open-policy-agent.github.io/gatekeeper/website/docs/) -
    [Network Policy
...
<AccordionGroup>
  <Accordion title="Database Performance">
    - [Dgraph Performance Tuning
    Guide](https://dgraph.io/docs/deploy/dgraph-administration/#performance-tuning)
    - [Dgraph Metrics and
    Monitoring](https://dgraph.io/docs/deploy/dgraph-administration/#metrics) -
    [Database Optimization Patterns](https://use-the-index-luke.com/) - [Query
    Performance Analysis](https://dgraph.io/docs/query-language/debugging/)
  </Accordion>

<Accordion title="Kubernetes Performance">
  - [Kubernetes Performance
  Tuning](https://kubernetes.io/docs/concepts/cluster-administration/system-logs/)
  - [Resource Management Best
  Practices](https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/)
  - [Node Performance
  Tuning](https://kubernetes.io/docs/tasks/administer-cluster/reserve-compute-resources/)
  - [Network Performance
  Optimization](https://kubernetes.io/docs/concepts/services-networking/network-policies/)
</Accordion>

  <Accordion title="Storage Performance">
    - [Storage Class
    Optimization](https://kubernetes.io/docs/concepts/storage/storage-classes/)
    - [Volume Performance
    Tuning](https://kubernetes.io/docs/concepts/storage/persistent-volumes/#performance)
    - [CSI Driver Best Practices](https://kubernetes-csi.github.io/docs/) -
    [Backup Performance
    Optimization](https://velero.io/docs/main/performance-guidance/)
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  <Card
    title="Velero Backup Solution"
    icon="shield-alt"
    href="https://velero.io/docs/"
  >
    Kubernetes native backup and disaster recovery
  </Card>
...
>
  ...
  >
    Continuous deployment using Git workflows
  </Card>
</CardGroup>
...
<CardGroup cols={3}>
  <Card title="Dgraph Slack" icon="slack" href="https://dgraph.slack.com/">
    Real-time community support and discussions
  </Card>

<Card title="CNCF Community" icon="users" href="https://www.cncf.io/community/">
  ...
  <Card
    title="SRE Community"
    icon="globe"
    href="https://www.usenix.org/conferences/srecon"
  >
    Site Reliability Engineering conferences and resources
  </Card>
</CardGroup>
...
<Tabs>
  <Tab title="Kubernetes">
    - [Kubernetes Academy](https://kube.academy/) - [CKAD
    Certification](https://www.cncf.io/certification/ckad/) - [CKA
    Certification](https://www.cncf.io/certification/cka/) - [Kubernetes the
    Hard Way](https://github.com/kelseyhightower/kubernetes-the-hard-way)
  </Tab>

<Tab title="Graph Databases">
  - [Graph Database Concepts](https://neo4j.com/developer/graph-database/) -
  [Graph Algorithms](https://github.com/neo4j-contrib/neo4j-graph-algorithms) -
  [GraphQL Best Practices](https://graphql.org/learn/best-practices/) -
  [Property Graph Model](https://github.com/opencypher/openCypher)
</Tab>

  <Tab title="SRE & Operations">
    - [Google SRE Book](https://sre.google/sre-book/table-of-contents/) - [Site
    Reliability Workbook](https://sre.google/workbook/table-of-contents/) -
    [Observability
    Engineering](https://www.oreilly.com/library/view/observability-engineering/9781492076438/)
    - [Building Secure and Reliable
    Systems](https://sre.google/static/pdf/building_secure_and_reliable_systems.pdf)
  </Tab>
</Tabs>
...
<AccordionGroup>
  <Accordion title="CLI Tools">
    ...
    command-line tool - [helm](https://helm.sh/) - Kubernetes package manager -
    [kubectx/kubens](https://github.com/ahmetb/kubectx) - Context and namespace
    switching - [k9s](https://k9scli.io/) - Terminal UI for Kubernetes -
    [stern](https://github.com/stern/stern) - Multi-pod log tailing - [dgraph
    CLI](https://dgraph.io/docs/deploy/cli-command-reference/) - Dgraph
    command-line tools
  </Accordion>
...
<Accordion title="Development Tools">
  ...
  [Garden](https://garden.io/) - Development orchestration tool -
  [DevSpace](https://devspace.sh/) - Developer tool for Kubernetes
</Accordion>

  <Accordion title="Networking Tools">
    - [Istio Service Mesh](https://istio.io/latest/docs/) - [Linkerd Service
    Mesh](https://linkerd.io/2.12/overview/) - [Cilium
    Networking](https://docs.cilium.io/en/stable/) - [Calico Network
    Policies](https://docs.projectcalico.org/security/kubernetes-network-policy)
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  <Card title="Plan Your Migration" icon="calendar">
    Use the decision matrix and resource calculators to plan your specific
    migration approach
  </Card>

<Card title="Join the Community" icon="users">
  Connect with other users who have completed similar migrations for advice and
  support
</Card>

<Card title="Monitor and Optimize" icon="chart-line">
  Implement comprehensive monitoring and continue optimizing performance
  post-migration
</Card>

  <Card title="Contribute Back" icon="heart">
    Share your migration experience and lessons learned with the community
  </Card>
</CardGroup>
