...
deleting data is done through **_DQL Mutations_**.
...
will add the `release_date` information to the node identified by UID
`0x632ea2`.
...
<Note>
  ...
  predicates that don't match one of the node's types will remain after an `S *
  *` delete mutation.
</Note>
...
<Note>
  The patterns `* P O` and `* * O` are not supported because it's inefficient to
...
- Using the [wildcard delete](#wildcard-delete) (star notation) mentioned in the
  last section.
- Setting the object to a specific value. If the value passed is not the current
  value, the mutation will succeed but will have no effect. If the value passed
  is the current value, the mutation will succeed and will delete the non-list
  predicate.
...
the state of the database _before mutation was executed_. To get the latest
...
the [uid](./uid-upsert) and [val](./val-upsert) functions.
