...
support them via [xid properties](./external-ids-upsert-block). When importing
RDF from another source that does not use numeric UID subjects, it will be
required to replace arbitrary subject IRIs with blank node IRIs.
