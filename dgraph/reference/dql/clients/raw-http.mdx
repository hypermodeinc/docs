...
It's also possible to interact with Dgraph directly via its HTTP endpoints. This
allows clients to be built for languages that don't have access to a working
gRPC implementation.
...
<Note>
  The request will update or create the predicates and types present in the
  request. It will not modify or delete other schema information that may be
  present.
</Note>
...
`/mutate` endpoint. Now that we have the current balances, we need to send a
mutation to Dgraph with the updated balances. If Bob transfers $10 to Alice,
then the RDFs to send are:
...
header to `application/rdf` in order to specify that mutation is written in RDF
format.
...
<Note>
  It's possible to commit immediately after a mutation is made (without
...
To abort a transaction, use the same `/commit` endpoint with the `abort=true`
...
[read-only](./go.md#read-only-transactions) query.
...
[best-effort](./go.md#read-only-transactions) query.
...
[GraphQL Variables](./query-language/graphql-variables) with the HTTP API.
