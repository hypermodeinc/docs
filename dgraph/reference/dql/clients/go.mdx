...
[this link](https://github.com/dgraph-io/dgo#supported-versions).
...
In [multi-tenancy](./multitenancy) environments, Dgraph provides a new method
`LoginIntoNamespace()`, which will allow the users to login to a specific
namespace.
...
<Note>
  The `dgo.DialSlashEndpoint()` method has been deprecated and will be removed
  in v21.07. Please use `dgo.DialCloud()` instead.
</Note>
...
The old way to send a drop all operation is still supported but will be
eventually deprecated. It is shown below for reference.
...
It shows how to to create a `Node` with name `Alice`, while also creating her
...
<Tip>
  ...
  repository](https://github.com/dgraph-io/dgo/blob/master/example_set_object_test.go).
</Tip>
