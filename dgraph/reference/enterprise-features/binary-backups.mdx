...
<Note>
  ...
  [v1.1.0](https://github.com/dgraph-io/dgraph/releases/tag/v1.1.0).
</Note>
...
the backup. Unlike [exports](./dgraph-administration.md#exporting-database),
...
Starting with
[v20.07.0](https://github.com/dgraph-io/dgraph/releases/tag/v20.07.0) if the
...
[whitelisted for admin operations](./dgraph-administration.md#whitelisting-admin-operations).
...
create a Service Account key for GCS and get a credentials file. See
...
if this flag is absent, it is presumed that the corresponding backup is not
encrypted.
...
Backup is an online tool, meaning it is available when Dgraph Alpha server is
...
<Note>
  ...
  Rest](./enterprise-features/encryption-at-rest) uses a symmetric-key algorithm
...
restores a backup to a new Dgraph cluster, so it is not designed to restore a
...
You use the `--vault` [superflag](./deploy/cli-command-reference) to specify the
