...
| Mode    | Behavior                                                  |
| ------- | --------------------------------------------------------- |
| `anyof` | Returns nodes that match on _any_ of the tokens generated |
| `allof` | Returns nodes that match on _all_ of the tokens generated |
...
terminology these are called _runes_).
...
- Inside `Tokens`, you can assume that `value` will have concrete type
  corresponding to that specified by `Type()`. It's safe to do a type assertion.
...
It's possible to search for people that have _any_ of the supplied runes in
their names (rather than _all_ of the supplied runes). To do this, use `anyof`
instead of `allof`:
