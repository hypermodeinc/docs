...
For sorting on predicates with
[sortable indices](./dql-schema.md#sortable-indices), Dgraph sorts on the values
and with the index in parallel and returns whichever result is computed first.
...
<Tip>
  ...
  with [first](./query-language/pagination.md#first).
</Tip>

Query Example: French director Jean-Pierre Jeunet's movies sorted by release
date.

````json { me(func: allofterms(name@en, "Jean-Pierre Jeunet")) { name@fr
director.film(orderasc: initial_release_date) { name@fr name@en
...
```json { genres as var(func: has(~genre)) { ~genre { numGenres as
count(genre) } }

genres(func: uid(genres), orderasc: name@en) { name@en ~genre (orderdesc:
val(numGenres), first: 5) { name@en genres : val(numGenres) } } }
```
