...
"og:title": "Advanced Queries with DQL - Dgraph Cloud"
---
...
[client library documentation](/dgraph/sdks/overview). To learn more about DQL,
see [DQL Schema](/dgraph/dql/schema).

<Note>
  Dgraph Cloud's [schema modes](./schema-modes) let you configure whether and
  how schema changes are allowed. To alter your schema using the `/alter` HTTP
...
create an API token, please see [Authentication](./authentication).
...
You can also access the [`/mutate`](/dgraph/dql/mutation) and `/commit`
endpoints.
...
[pydgraph client](https://github.com/hypermodeinc/pydgraph) to make gRPC
requests.
...
2. Enter your host in the Dgraph Server URL field. This is obtained by removing
   `/graphql` from the end of your `/graphql` endpoint URL. For example, if your
   `/graphql` endpoint is
...
Ratel has certain limitations, such as not supporting backups, modifying Access
Control Lists (ACL), or attempting to remove nodes from the cluster.
...
backend to flexible mode. To learn more, see [Schema Modes](./schema-modes).
