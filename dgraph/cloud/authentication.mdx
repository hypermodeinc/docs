...
with [anonymous access](dgraph/graphql/security/anonymous-access) not set on the
requested operation, must present a valid **client** or **admin API key** in the
`Dg-Auth` or `X-Auth-Token` header of every HTTP request.
