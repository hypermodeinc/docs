...
"og:title": "Importing and Exporting data from Dgraph Cloud - Dgraph Cloud"
---
...
1.  Obtain dgraph binary or the latest docker image by following the
    [installation](/dgraph/self-managed/download) instructions. This is required
    to run Dgraph CLI command `dgraph live`.

2.  Obtain the `gRPC endpoint` of your Dgraph Cloud backend and a valid
    `Client API key`.

    An administrator gets those information with the following steps:

    - Log into the Dgraph Cloud account, select the backend
    - In the `Admin` section of the Dgraph Cloud console, go to `Settings` and
      copy the value of the `gRPC Endpoint` from the `General` tab.
    - Access the `API Keys` tab to generate an `Client API Key`.

           <Note>

      The gRPC endpoint is different from the `GraphQL endpoint`. The gRPC
      endpoint looks like `<domain>:443`
...
2. Make a note of the GraphQL endpoint for the instance from `Overview` in the
   Dgraph Cloud console. Replace `/graphql` with `/admin/slash`in the GraphQL
   endpoint to get the `<ADMIN_ENDPOINT>`.
3. Authenticate the `admin` API requests by adding the `<APIKEY>` as the
...
7. To get the status of export and the signed URLs to download the exported
...
[Cloud API documentation](/dgraph/cloud/api/backup#export-data).
...
- Cluster-wide export: this is an export of the entire backend (including all
  ...
  [_Guardian of Galaxy_](/dgraph/enterprise/multitenancy#guardians-of-the-galaxy)
  users.
