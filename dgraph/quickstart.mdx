...
This is a quick start guide to run [DQL](./dgraph-glossary.md#RDF) queries and
mutations. For an interactive walkthrough, take the
[tour](https://dgraph.io/tour/).
...
You can Login to Dgraph cloud using **Sign in with Google**, **Sign in with
GitHub** or any other email account that you prefer to use.
...
1. Click **Launch**
1. Click **Ratel** to access the UI that provides browser-based queries,
...
The input data is in JSON Format. Dgraph also supports
[RDF](./dgraph-glossary.md#RDF) notation.

The sample JSON data is an array of two movies with some attributes. These are
stored as [Nodes](./dgraph-glossary.md#node) in Dgraph.
...
`starring` field which is an array of JSON objects. Each object is also stored
as a Node in Dgraph . The `director` and `starring` are stored as
[relations](./dgraph-glossary.md#relation).
...
Dgraph displays the universal identifiers ([UID](./dgraph-glossary.md#uid)) of
...
The query lists all movies that have a `release_date` and for each, it looks for
the `director` and `starring` relations and provides the name attribute of the
related nodes if any.

2.  In the response panel, select **Graph**, to view a Graph output:

![Query result](/images/dql-quickstart/img1.png)
...
1.  In the **Schema** page, select **Predicates**. Dgraph creates and displays
    ...
    [predicate](./dgraph-glossary.md#predicate) is Dgraph internal
    representation of a node attribute or a relation.
...
![Adding an index](/images/dql-quickstart/predicate-name.png)
...
Let's get the movies having the term "Star" in their name and released before
"1979".
...
- A wider range of queries can also be found in the
  [Query Language](/query-language/index) reference.
