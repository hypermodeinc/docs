...
- **Fully Transactional and ACID-compliant**: Dgraph satisfies demanding
  transactional workloads that require frequent inserts and updates, with
  guarantees for atomicity, consistency, isolation, and durability (ACID)

- **Language support & Text Search**: Full-text searching is included and
  strings can be expressed in multiple languages.

- **Geolocation data and queries**: Dgraph supports points and shapes data, and
  queries can use near, within, contains, or intersects functions.
...
In Dgraph, GraphQL isn't an afterthought or an add-on–it's core to the product.
...
- **Dgraph Alpha** server nodes store the graph data and indices. Unlike
  ...
  data sets.
  ![Dgraph Internal Architecture](/images/dgraph/dgraph-architecture.png)
...
- **Get Started** with a [free database instance](https://hypermode.com/sign-up)
