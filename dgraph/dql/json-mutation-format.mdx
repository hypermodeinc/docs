...
When using JSON format Dgraph creates nodes and relationships from the JSON
structure and assigns UIDs to nodes.
...
In this mutation, there are two JSON objects and because they are referring to
the same identifier, Dgraph creates only one node:
...
To set a string value for a specific lnguage, append the language tag to the
...
misinterpreted as a time value, it's best to store numeric data as either an int
or a float.
...
respective facets value as map values. Predicate values which does not have
facets values will be missing from facets map. E.g.
...
Above you see that we have three values ​​to enter the list with their
