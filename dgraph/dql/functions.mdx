...
[variables](./variables#value-variables.md). Functions can be applied in the
query root or in filters.

Comparison functions (`eq`, `ge`, `gt`, `le`, `lt`) in the query root (`func:`)
can only be applied on [indexed predicates](./schema#indexing). Since v1.2,
comparison functions can now be used on
...
[count index](/schema#count-index) records the number of edges out of nodes and
