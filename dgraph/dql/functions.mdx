...
can only be applied on [indexed predicates](./dql-schema.md#indexing). Since
v1.2, comparison functions can now be used on
[@filter](./query-language/graphql-fundamentals.md#applying-filters) directives
even on predicates that haven't been indexed. Filtering on non-indexed
