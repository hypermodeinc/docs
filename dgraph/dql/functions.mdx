...
[variables](./query-language/value-variables.md). Functions can be applied in
the query root or in filters.

<Note>
  Support for filters on non-indexed predicates was added with Dgraph `v1.2.0`.
</Note>

Comparison functions (`eq`, `ge`, `gt`, `le`, `lt`) in the query root (aka
`func:`) can only be applied on [indexed predicates](./dql-schema.md#indexing).
Since v1.2, comparison functions can now be used on
[@filter](./query-language/graphql-fundamentals.md#applying-filters) directives
even on predicates that have not been indexed. Filtering on non-indexed
predicates can be slow for large datasets, as they require iterating over all of
the possible values at the level where the filter is being used.
...
also meet the search terms because without a
[cascade directive](./query-language/cascade-directive.md) the query doesn't
require a genre.
...
`ryan` but regular expression search will also match within terms, such as
`bryan`.
...
[trigram indexing](https://swtch.com/~rsc/regexp/regexp4.html). That is, Dgraph
converts the regular expression to a trigram query, uses the trigram index and
trigram query to find possible matches and applies the full regular expression
search only to the possibles.
...
- At least one trigram must be matched by the regular expression (patterns
  shorter than 3 runes are not supported). That is, Dgraph requires regular
...
- The number of alternative trigrams matched by the regular expression should be
  as small as possible (`[a-zA-Z][a-zA-Z][0-9]` is not a good idea). Many
...
- If repeat specifications (`*`, `+`, `?`, `{n,m}`) are used, the entire regular
  expression must not match the _empty_ string or _any_ string: for example, `*`
...
the string, also known as _fuzzy matching_. The distance parameter must be
...
is no order specified on genres, the order will be by UID. The [count index](/dql-schema.md#count-index) records the number of edges out of nodes and
makes such queries more.
...
[SF Tourism](https://github.com/dgraph-io/benchmarks/blob/master/data/sf.tourism.gz?raw=true)
dataset.
...
San Francisco Zoo. ```json { tourist(func: contains(loc, [
-122.50326097011566, 37.73353615592843 ] )) { name } } ```
