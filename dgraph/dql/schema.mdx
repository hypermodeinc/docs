...
[`similar_to`](./functions#vector-similarity-search) function can be used for
similarity search.
...
The schema is also used to set [predicates indexes](#predicate-indexing) which
are required to apply [filtering functions](./functions) in DQL queries.
