...
- At most one result will be returned (except in the case where the language
  list is set to \*).
- The preference list is considered left to right: if a value in given language
  is not found, the next language from the list is considered.
...
- Setting the language list value to \* will return all the values for that
...
- `name@*` => Look for all the values of this predicate and return them along
  ...
  and hi, this query will return two keys named "name@en" and "name@hi".
...
````json { q(func: allofterms(name@en, "Farhan Akhtar")) { name@hi
name@en
