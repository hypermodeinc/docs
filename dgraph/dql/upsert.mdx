...
or an existing node is modified. It is not allowed that two concurrent upserts
...
single mutation. The request will contain both the query and the mutation as
explained [here](./dql-mutation.md#Update data with upsert block).

In GraphQL, you can use the `upsert` input variable in an `add` mutation, as
explained [here](./graphql/mutations/upsert.md).
...
<Note>
  It is the _index_ that typically causes upsert conflicts to occur. The index
...
[Conditional Upsert](./dql-mutation.md#conditional-upsert).
