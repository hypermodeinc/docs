...
use `expand()`, the [type system](./dql-schema) is required. Refer to the
section on the type system to check how to set the types nodes. The rest of this
section assumes familiarity with that section.
...
<Note>
  ...
  preference](./query-language/graphql-fundamentals.md#language-support)). So
  it's often required to add `name@fr` or `name@.` as well to an expand query.
</Note>
