...
[Dgraph JavaScript gRPC client](./grpc) closely.
...
<Note>
  You need to remove the `/graphql` path when copying the endpoint URL from the
...
1. Alpha servers can be configured with
   [Secure Alter Operations](/dgraph-administration). In this case the token
   needs to be set on the client instance:
