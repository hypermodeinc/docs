...
A minimal implementation for a Dgraph client for Java 1.11 and above, using
...
If [ACL](/dgraph/enterprise/access-control-lists) is enabled then you can log-in
to the default namespace (`0`) with the following method:
...
login method on client will login into the namespace `0`. In order to login into
some other namespace, use the `loginIntoNamespace` method on the client:
