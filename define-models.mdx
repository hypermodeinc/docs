...
When using `hugging-face` as the `provider` and `hypermode` as the `host`, Hypermode will automatically deploy a dedicated instance of the defined `sourceModel` when deploying your project. Your project's functions will securely connect to the hosted model, with no additional configuration required.