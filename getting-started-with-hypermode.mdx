---
title: Getting Started with Hypermode
description: "Learn how to instantly get strated with Hypermode."
---

This guide will walk you through getting started instantly with Hypermode using the [Instant Vector Search template](https://github.com/hypermodeAI/hyper-commerce). You’ll also learn how to customize your functions to tailor the application to your needs.

# What is Hypermode?

[Hypermode](https://docs.hypermode.com/introduction) is an application framework designed to simplify the development and deployment of AI features and assistants. With its [Functions SDK](https://docs.hypermode.com/sdk/functions-sdk) and runtime, developers can quickly deploy intelligent features into production without the need for extensive data wrangling. Hypermode supports rapid iteration, allowing you to start with a large language model and later fine-tune smaller, open-source models. Its production-ready GraphQL API auto-generates and enables seamless integration. Hypermode is ideal for developers looking to quickly prototype and evolve AI capabilities.
...
1. Visit the [Hypermode website](https://hypermode.com/sign-in) and sign up with your GitHub, Google, or email address. Once signed in, you'll be taken to your Hypermode projects dashboard.
...
4. You will be prompted to choose between using sample data or your own data:
...
5. Once created, a runtime is provisioned with instant connectivity to shared AI models that bring the functions within your project to life. The source code for the project is open source and can be viewed in [this repository](https://github.com/hypermodeAI/hyper-commerce).
...
If you selected sample data, no additional setup is required. You can move on to Step 3 and immediately start exploring the project and its features. If you chose to use your own data, follow these steps to seed your data into the collections:
...
2. Prepare your CSV file with the following headers: `Uniq Id,Product Name,Category,Selling Price,About Product,Image,Num Stars,In Stock`.
...
- [\*\*Functions](https://docs.hypermode.com/sdk/functions-sdk):\*\* These are serverless functions written in AssemblyScript (a TypeScript-like language) that are automatically exposed as GraphQL APIs. Once deployed, these functions can be queried within your application.
- [\*\*Collections](https://docs.hypermode.com/sdk/collections):\*\* Hypermode offers built-in key-value storage, known as collections, with support for vector embeddings. This allows you to store and retrieve data efficiently, without requiring a database for basic use cases.
- [\*\*Models](https://docs.hypermode.com/sdk/models):\*\* This section represents the AI models defined for your project. These models handle tasks like embedding text into vectors for search. Hypermode provides open source shared and dedicated model hosting for rapid experimentation. You can also connect to your preferred LLM, including OpenAI, Anthropic, and Gemini.
- [\*\*Hosts](https://docs.hypermode.com/define-hosts#define-hosts):\*\* All external connections from your functions are declared as hosts, with the runtime denying all other egress for secure-by-default processing.
- **Endpoint:** You will also see the GraphQL endpoint, which you’ll use to interact with your APIs and query your data.

### Step 4: Querying Your Data

Now that your project is set up, deployed, and seeded with data, you can test your functions using the query interface in the Hypermode console.
...
3. You should see the data for 4 items returned from the `searchProducts` endpoint that match your query, “shoe.” Feel free to experiment with the query—adjust the `maxItems` value to return more items, or change the search query to see how the returned data matches your input. Additionally, notice that items are ranked based on their star rating and whether they are in stock.
...
Now that you've set up your project and queried your data in the console, you can test the functionality in a frontend UI.
...
5. With the project running locally, you can now test the search functionality in the provided UI. Try searching for products to see how the data from your Hypermode project is integrated and displayed.

> Note: This quickstart is intended as a proof of concept. For more advanced usage, such as customizing search re-ranking logic, you'll need to clone the template to your own repository to make and push changes. Refer to the next section for further instructions.
...
2. Clone the repo by clicking "Use this template" in the upper-right corner and selecting "Create new repo." This will clone the code into your own GitHub repository
3. Visit the [Hypermode website](https://hypermode.com/sign-in) and sign up with your GitHub, Google, or email address. Once signed in, you'll be taken to your Hypermode projects dashboard.
4. In the Hypermode console, click "New Project".
5. Select "Import GitHub Repo"
...
8. Once deployed, your functions and collections will be visible in the Hypermode console.
...
8. The script will batch insert the data and display the time taken for each operation. Inserting the full dataset (10,000 rows) may take around 18 minutes. If you want to test with a smaller dataset, feel free to reduce the size of the CSV.
...
If you'd like to rank products based solely on their star rating, without considering whether they are in stock, follow these steps:
...
2. Any commit to the `main` branch will automatically trigger a Hypermode deployment.
3. After deployment, when you query the `searchProducts` endpoint again, it will rank products solely based on their star rating.

## Testing your functions

Once you’ve made changes to your backend functions and deployed them to Hypermode, it's time to test the updates.

1. **Test in the Query Tab:**

   In the Hypermode console, navigate to the Query tab to test your modified functions directly. Run queries similar to the ones you used earlier to ensure the changes are reflected in the results.
...
2. Run the Frontend Locally:
   ...
   > Note: Both of these values are found in the Hypermode console

   Next, just run the command `npm run dev` in your terminal to run the application locally. Now you can test the changes you made to your backend functions.
