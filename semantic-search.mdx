...
to add natural language or semantic search to your app.
...
- **Hypermode** is he platform for deploying AI applications, in this case we
  leverage Hypermode's hosted models
...
Now we're ready to create Modus functions that creates data in Dgraph. Here we
...
computing an embedding of the description of an input product.
...
can execute the following mutation, using the value of our search string for the
`$search` variable.
...
next step and deploy our project to Hypermode. See the Deploy Project section
for a walk through of this process.
