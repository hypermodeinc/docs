...
<Warning>
  TO DO: verify CLI URLs and internal reference links Verify CLI commands Add
  screenshots
</Warning>
...
If you've never used Hypermode before, sign up for a new Hypermode account
...
After deploying your project, go to your [Hypermode dashboard](https://hypermode.com/go). You can run a few sample queries in the web console to verify it's working as expected. In the following query, we're going to use the `generateText` function to generate text from the shared Meta Llama 3.1 model based on the prompt "How are black holes created?"
