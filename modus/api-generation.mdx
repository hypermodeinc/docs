...
Modus automatically creates an external API based on the endpoints defined in your
[app manifest](/modus/app-manifest#endpoints). Modus generates the API signature based on the
functions you export from your app.
...
Since the `classify` function isn't capitalized, Modus doesn't include it in the generated GraphQL
API.
...
Since the `classify` function isn't exported from the module, Modus doesn't include it in the
