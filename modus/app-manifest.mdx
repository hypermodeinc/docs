...
Each connection has a `type` property, which controls how it's used and which additional properties are available. The
...
This connection type supports the HTTP and HTTPS protocols to communicate with external hosts. You can use the
[HTTP APIs](/modus/sdk/http) in the Modus SDK to interact with the host.

This connection type is also used for [GraphQL APIs](/modus/sdk/graphql) and to invoke externally hosted AI
...
each connection, via the Hypermode Console.
...
This connection type supports connecting to Dgraph databases. You can use the [Dgraph APIs](/modus/sdk/dgraph) in the
Modus SDK to interact with the database.
