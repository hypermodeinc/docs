...
The manifest in a Hypermode project allows you to configure the resources your functions have access to at runtime. You
define the manifest in the `hypermode.json` file within the root of your project directory.
...
Each host has a `type` property, which controls how it's used and which additional properties are available. If not
provided, the default type is `http`. The following table lists the available host types:
