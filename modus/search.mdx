...
- **Vetor embedding**: In order to be used for vector-based search and comparison, each item in the collection must be converted into a vector representation called **embedding**. By embedding data, you enable powerful natural language and similarity-based searches. When configured, Modus runtime automatically compute the embeddings when items are added or updated.
...
- export an embedding functio

  n

- declare a search method

### Export an embedding function

An embedding function in any function that transforms text into vectors that represent their meaning in a high-dimensional space.

Embeddings functions must be exported from the index.ts file of your project and have the following signature
...
TO DO introduce examples for OpenAI and minilm
