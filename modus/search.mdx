...
3. [**Using Dgraph's vector search functionality**](#natural-language-search-with-dgraph-and-modus)
...
4. [**Using Modus's built-in Collection data structure**](#natural-language-search-with-modus-collections)
...
### Setting up Dgraph connection

First, configure your Dgraph connection in the modus.json file:

```json
{
  "connections": {
    "dgraph-grpc": { "type": "dgraph", "grpcTarget": "localhost:9080" }
  }
}
```
...
Implement functions to retrieve, delete, and query products:
...
Define your Dgraph schema with vector indexing support by adding the
`@index(hnsw)` directive to the property storing the embedding value.
...
You can use Dgraph's Ratel UI to query your data. Here's an example of a vector
