...
func ImportAgentSighting(sighting AgentSighting) (*string, error) {
    ...
    mutation := dgraph.NewMutation().WithSetJson(fmt.Sprintf(`{
        ...
        "ai_analysis": "%s"
        // ...
    }`, sighting.SightingID, sighting.AgentName, sighting.Behavior, analysis))
...
Now that we've Agent sighting data in our knowledge graph, let's analyze the
broader threat landscape. This function queries relationships across all stored
Agent data:
...
Once we've the got institutional data, we use AI to analyze patterns across all
related sightings:
...
func generateThreatAssessment(sightings interface{}) (string, error) {
    ...
    input, _ := model.CreateInput(
        openai.NewSystemMessage(prompt),
        openai.NewUserMessage(fmt.Sprintf("Agent surveillance data: %s", string(sightingsJson))),
    )
...
### Step 5: checking agent operational status
