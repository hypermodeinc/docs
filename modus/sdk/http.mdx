...
`hypermodeinc/functions-as` repository, at [/examples/http](https://github.com/hypermodeinc/modus/tree/main/sdk/assemblyscript/examples/http).
...
By default, the `RequestOptions` contains an empty `Headers` object
which you can add headers to using the `append` method.
...
Content to pass in the request body, as a [`Content`](#content) object,
or `null` (the default) if there is no body to pass.
...
<Tip>
  The response body isn't normally read directly. Instead, use the `text` or
  `json` functions. You should only read the `body` directly if you expect to
