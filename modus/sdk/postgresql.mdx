...
`hypermodeinc/functions-as` repository, at [/examples/postgresql](https://github.com/hypermodeinc/modus/tree/main/sdk/assemblyscript/examples/postgresql).
...
Execute a SQL statement against a PostgreSQL database, without any data returned.
Use this for insert, update, or delete operations, or for other SQL statements that don't return data.
...
In the results, each row converts to an object of type `T`, with fields matching the column names.
...
Execute a SQL statement against a PostgreSQL database, returning a single scalar value.
For example, the result could be a count, sum, or average, or it could be an identifier.
...
To use this feature, create a new `Params` object and call the `push` method
...
The sequence of calls to `push` determines the order of the parameters in the
SQL statement. This corresponds to the order of the `?` placeholders or `$1`, `$2`, etc.
...
boolean values, arrays, maps, and custom objects decorated with `@json`, as long
as the database supports it.
...
`Params` object. You typically don't need to call it directly.
...
Also serves as the base class for `QueryResponse<T>` and `ScalarResponse<T>`.
