...
<ResponseField name="opts" type="Neo4jOption" required>
  Optional arguments to pass to the query. It is used to specify the database
  name using the `WithDbName` option.
</ResponseField>
...
Get the value of a record at a given key as a string.
...
An interface representing possible entities in a Neo4j query result. This
interface is implemented by `Node`, `Relationship`, and `Path`.
