...
There are no generics yet, but a similar effect can be done with Generics
...
To discern the type of a generic, AssemblyScript provides some helpful [Builtins]()
...
The `number` type in TypeScript can only be a [Float64](). In AssemblyScript, we can define which type of number we want to work with.
...
It is recommended that you use `i32`
...
It is recommended that you use `f64`
...
Primary functions, aka the `exported` functions in your entry file only support the following types as input parameters:
...
If a field is defined as nullable, such as name: string | null , or if it has a default value, such as age: i32 = 21 , it currently won’t currently allow the null (or absence of the field), in the GraphQL query.  This is a bug.
