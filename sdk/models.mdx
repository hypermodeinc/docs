...
Each demonstrates using different types of AI models for different purposes.
However, the Models interface isn't limited to these purposes. You can use it for any
task that an AI model can perform.

<Note>
  Currently, the models interface isn't designed for models that require
  streaming of either request or response content. We plan to address this
...
Refer to the `models-as` repository and the examples in the `functions-as` repository
