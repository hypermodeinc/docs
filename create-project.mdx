...
- **Project endpoint**: The structure of the project's endpoint is `<project-slug>-<org-slug>.hypermode.app`. This unique endpoint allows you to interact with your project's data and capabilities by hitting `/graphql`. For more details on integrating with the API, see [Integrate API](/integrate-api).
...
1. **Select an organization**  
   ...
   - **Existing organizations**: For organizations with existing projects, use the "New Project" button located on the right side of the organization’s **Projects** tab.

2. **Choose a template**  
   Select a template that best matches your use case or start with the [Base Template](https://github.com/hypermodeinc/base-template) to configure your project from scratch. Available options include:
   ...
   - **Modus Git repository import**: Import a Modus-based Git repository to integrate with your existing project workflows. Follow the prompts to connect with [GitHub](https://github.com/), then select the repository you'd like to import.
...
- Access the project’s GraphQL endpoint (`<project-slug>-<org-slug>@hypermode.app/graphql`) from your project's **Home**.
